  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
      ...
      <section className="space-y-5 lg:col-span-1">
        <h2 className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-2">Datas e Pessoas</h2>

        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-bold text-slate-800 mb-1">Ida</label>
            <input type="date" value={dataInicio} onChange={(e) => setDataInicio(e.target.value)}
              className="w-full p-3 bg-slate-50 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-sky-500 outline-none" />
          </div>
          <div>
            <label className="block text-sm font-bold text-slate-800 mb-1">Volta</label>
        ...
        <div>
          <label className="block text-sm font-bold text-slate-800 mb-1">Número de Pessoas</label>
          <input type="text" value={numPessoas} onChange={(e) => setNumPessoas(e.target.value)} placeholder="2"
            className="w-full p-3 bg-slate-50 border border-slate-300 rounded-xl text-base focus:ring-2 focus:ring-sky-500 outline-none" />
        </div>

        <h2 className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-2 pt-2">Custos</h2>

        <div>
          <label className="block text-sm font-bold text-slate-800 mb-1">Passagem por Pessoa (R$)</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-bold text-slate-800 mb-1">Diária Hotel (R$)</label>
            <input type="text" value={diariasHotel} onChange={(e) => setDiariasHotel(e.target.value)} placeholder="200"
              className="w-full p-3 bg-slate-50 border border-slate-300 rounded-xl text-base focus:ring-2 focus:ring-sky-500 outline-none" />
          </div>
          <div>
            <label className="block text-sm font-bold text-slate-800 mb-1">Noites</label>
        ...
        <div>
          <label className="block text-sm font-bold text-slate-800 mb-1">Alimentação por Pessoa/Dia (R$)</label>
          <input type="text" value={gastoAlimentacao} onChange={(e) => setGastoAlimentacao(e.target.value)} placeholder="80"
            className="w-full p-3 bg-slate-50 border border-slate-300 rounded-xl text-base focus:ring-2 focus:ring-sky-500 outline-none" />
        </div>

        <div>
          <label className="block text-sm font-bold text-slate-800 mb-1">Transporte Local/Dia (R$)</label>
          <input type="text" value={gastoTransporte} onChange={(e) => setGastoTransporte(e.target.value)} placeholder="30"
            className="w-full p-3 bg-slate-50 border border-slate-300 rounded-xl text-base focus:ring-2 focus:ring-sky-500 outline-none" />
        </div>

        <div>
          <label className="block text-sm font-bold text-slate-800 mb-1">Atividades/Passeios (R$)</label>
          <input type="text" value={gastoAtividades} onChange={(e) => setGastoAtividades(e.target.value)} placeholder="300"
            className="w-full p-3 bg-slate-50 border border-slate-300 rounded-xl text-base focus:ring-2 focus:ring-sky-500 outline-none" />
        </div>

        <div>
          <label className="block text-sm font-bold text-slate-800 mb-1">Outros Gastos (R$)</label>
      ...
      <section className="lg:col-span-2 space-y-6 bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-200">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-white p-4 rounded-xl border border-slate-200 text-center">
            <p className="text-xs font-bold text-slate-400 uppercase mb-1">Dias Úteis</p>
            <p className="text-3xl font-black text-slate-900">{resultado.diasUteis}</p>
          </div>
          <div className="bg-white p-4 rounded-xl border border-slate-200 text-center">
            <p className="text-xs font-bold text-slate-400 uppercase mb-1">Faltam</p>
            <p className="text-3xl font-black text-sky-600">{resultado.diasParaViagem}</p>
          </div>
        </div>

        {/* Totais */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4 overflow-hidden">
          <div className="bg-white p-3 sm:p-4 rounded-xl border border-slate-200 shadow-sm text-center flex flex-col justify-center overflow-hidden">
          ...
          <div className="bg-white p-3 sm:p-4 rounded-xl border border-slate-200 shadow-sm text-center flex flex-col justify-center overflow-hidden">
            ...
            <p className="text-lg sm:text-xl lg:text-base xl:text-xl font-black text-sky-600 tracking-tighter whitespace-nowrap overflow-hidden text-ellipsis" title={formatCurrency(resultado.custoPorPessoa)}>{formatCurrency(resultado.custoPorPessoa)}</p>
          </div>
          <div className="bg-white p-3 sm:p-4 rounded-xl border border-slate-200 shadow-sm text-center flex flex-col justify-center overflow-hidden">
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
          <h3 className="text-base font-bold text-slate-900 mb-4">Distribuição de Gastos</h3>
